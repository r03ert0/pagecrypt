...
<head>
  ...
  <script type="module">
//assets/index.788f565d.js
const N=function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))n(r);new MutationObserver(r=>{for(const s of r)if(s.type==="childList")for(const o of s.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function a(r){const s={};return r.integrity&&(s.integrity=r.integrity),r.referrerpolicy&&(s.referrerPolicy=r.referrerpolicy),r.crossorigin==="use-credentials"?s.credentials="include":r.crossorigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function n(r){if(r.ep)return;r.ep=!0;const s=a(r);fetch(r.href,s)}};N();function O(t,e,a){var n;if(a===void 0&&(a={}),!e.codes){e.codes={};for(var r=0;r<e.chars.length;++r)e.codes[e.chars[r]]=r}if(!a.loose&&t.length*e.bits&7)throw new SyntaxError("Invalid padding");for(var s=t.length;t[s-1]==="=";)if(--s,!a.loose&&!((t.length-s)*e.bits&7))throw new SyntaxError("Invalid padding");for(var o=new((n=a.out)!=null?n:Uint8Array)(s*e.bits/8|0),i=0,l=0,f=0,d=0;d<s;++d){var b=e.codes[t[d]];if(b===void 0)throw new SyntaxError("Invalid character "+t[d]);l=l<<e.bits|b,i+=e.bits,i>=8&&(i-=8,o[f++]=255&l>>i)}if(i>=e.bits||255&l<<8-i)throw new SyntaxError("Unexpected end of data");return o}function P(t,e,a){a===void 0&&(a={});for(var n=a,r=n.pad,s=r===void 0?!0:r,o=(1<<e.bits)-1,i="",l=0,f=0,d=0;d<t.length;++d)for(f=f<<8|255&t[d],l+=8;l>e.bits;)l-=e.bits,i+=e.chars[o&f>>l];if(l&&(i+=e.chars[o&f<<e.bits-l]),s)for(;i.length*e.bits&7;)i+="=";return i}var S={chars:"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/",bits:6},T={parse:function(e,a){return O(e,S,a)},stringify:function(e,a){return P(e,S,a)}};const x=document.querySelector.bind(document),[c,p,I,y,h]=["input","header","#msg","form","#load"].map(x);let g,E,L;const M=async()=>{const t=x("pre").innerText;if(!t){c.disabled=!0,m("No encrypted payload.");return}const e=T.parse(t);g=e.slice(0,32),E=e.slice(32,32+16),L=e.slice(32+16),localStorage.passphrase&&(c.value=localStorage.passphrase),sessionStorage.k||c.value?await k():(w(h),v(y),p.classList.replace("hidden","flex"),c.focus())};var K,A;const u=((K=window.crypto)==null?void 0:K.subtle)||((A=window.crypto)==null?void 0:A.webkitSubtle);u||(m("Please use a modern browser."),c.disabled=!0);function v(t){t.classList.remove("hidden")}function w(t){t.classList.add("hidden")}function m(t){I.innerText=t,p.classList.add("text-red-600")}y.addEventListener("submit",async t=>{t.preventDefault(),await k()});async function C(t){return new Promise(e=>setTimeout(e,t))}async function k(){localStorage.passphrase=c.value,h.lastElementChild.innerText="Loading...",w(p),w(y),v(h),await C(60);try{const t=await G({salt:g,iv:E,ciphertext:L},c.value);document.write(t),document.close()}catch(t){w(h),v(y),p.classList.replace("hidden","flex"),sessionStorage.k?sessionStorage.removeItem("k"):m("Wrong password."),c.value="",c.focus()}}async function D(t,e){const a=new TextEncoder,n=await u.importKey("raw",a.encode(e),"PBKDF2",!1,["deriveKey"]);return await u.deriveKey({name:"PBKDF2",salt:t,iterations:2e6,hash:"SHA-256"},n,{name:"AES-GCM",length:256},!0,["decrypt"])}async function F(t){return u.importKey("jwk",t,"AES-GCM",!0,["decrypt"])}async function G({salt:t,iv:e,ciphertext:a},n){const r=new TextDecoder,s=sessionStorage.k?await F(JSON.parse(sessionStorage.k)):await D(t,n),o=new Uint8Array(await u.decrypt({name:"AES-GCM",iv:e},s,a));if(!o)throw"Malformed data";return sessionStorage.k=JSON.stringify(await u.exportKey("jwk",s)),r.decode(o)}M();<\/script>
...
async function encryptHTMLCustom(inputHTML, password, promptTemplate) {
  return promptTemplate.replace(/<!--ENCRYPTED PAYLOAD-->/, `<pre class="hidden">${await getEncryptedPayload(inputHTML, password)}</pre>`);
}
